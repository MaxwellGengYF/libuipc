...
{
    on_init_scene([this] { m_impl.init(world(), engine().device(), engine().stream()); });
...
void HalfPlane::Impl::_build_geometry(luisa::compute::Device& device,
                                      luisa::compute::Stream& stream)
{
    ...
    auto I2G = flattener.compute_instance_to_geometry();
    ...
    if(instance_count > 0)
    {
        positions = device.create_buffer<Vector3>(h_positions.size());
        normals = device.create_buffer<Vector3>(h_normals.size());

        stream << positions.copy_from(h_positions.data())
               << normals.copy_from(h_normals.data());
    }
...
luisa::compute::BufferView<const Vector3> HalfPlane::normals() const
{
    return m_impl.normals.view();
}

luisa::compute::BufferView<const Vector3> HalfPlane::positions() const
{
