...
class GeometryInstanceFlattener
{
  public:
    ...
    vector<size_t> compute_instance_to_geometry() const
    {
        vector<size_t> map;
    ...
    /**
     * @brief Create a buffer for flattened instance data on the device.
     * 
     * @tparam T The element type
     * @param device The LuisaCompute device
     * @param stream The command stream for upload operations
     * @return Buffer<T> A device buffer with size equal to instance count
     */
    template <typename T>
    luisa::compute::Buffer<T> create_instance_buffer(luisa::compute::Device& device,
                                                     luisa::compute::Stream& stream) const
    {
        return device.create_buffer<T>(compute_instance_count());
    }
    ...
    /**
     * @brief Upload instance data to a LuisaCompute buffer.
     * 
     * @tparam T The element type
     * @param stream The command stream
     * @param buffer The device buffer to upload to
     * @param getter A callable that returns host data for each instance
     */
    template <typename T, typename Getter>
    void upload_to_buffer(luisa::compute::Stream& stream,
                          luisa::compute::Buffer<T>& buffer,
                          Getter&& getter) const
    {
        // First collect all data on host
        vector<T> host_data;
        host_data.reserve(compute_instance_count());
        ...
        for(auto geo : m_geos)
        {
            ...
            for(auto J : range(N))
            {
                host_data.push_back(getter(geo, J));
            }
        }
        ...
        if(!host_data.empty())
        {
            stream << buffer.copy_from(host_data.data()) << luisa::compute::synchronize();
        }
    }

    /**
     * @brief Get a view of the geometries.
     * 
     * @return span<GeometryT*> The span of geometry pointers
     */
    span<GeometryT*> geometries() const noexcept
    {
        return m_geos;
    }
