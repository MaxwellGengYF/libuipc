...
{
template <>
class backend::SimSystemCreator<luisa::InterPrimitiveConstitutionManager>
{
  public:
    static U<luisa::InterPrimitiveConstitutionManager> create(SimEngine& engine)
    {
        ...
        return uipc::make_unique<luisa::InterPrimitiveConstitutionManager>(engine);
    }
};
}  // namespace uipc::backend
...
luisa::compute::BufferView<Float> InterPrimitiveConstitutionManager::EnergyInfo::energies() const noexcept
{
    auto [offset, count] = m_impl->constitution_energy_offsets_counts[m_index];
    return m_energies.subview(offset, count);
}

MutableDoubletVectorView<Float, 3> InterPrimitiveConstitutionManager::GradientHessianInfo::gradients() const noexcept
{
    auto [offset, count] = m_impl->constitution_gradient_offsets_counts[m_index];
    return m_gradients.subview(offset, count);
}

MutableTripletMatrixView<Float, 3> InterPrimitiveConstitutionManager::GradientHessianInfo::hessians() const noexcept
{
...
luisa::compute::BufferView<const Vector3> InterPrimitiveConstitutionManager::BaseInfo::positions() const noexcept
{
