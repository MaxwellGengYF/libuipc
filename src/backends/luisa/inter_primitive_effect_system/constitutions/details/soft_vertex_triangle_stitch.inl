...
// Custom types for 9-element vector and 9x9 matrix (Luisa doesn't have these)
template <typename T>
struct Vector9 {
    T v[9];
    
    [[nodiscard]] constexpr T& operator()(size_t i) noexcept { return v[i]; }
    [[nodiscard]] constexpr const T& operator()(size_t i) const noexcept { return v[i]; }
};
...
struct Matrix9x9 {
    ...
    [[nodiscard]] constexpr T& operator()(size_t row, size_t col) noexcept { return m[row][col]; }
    [[nodiscard]] constexpr const T& operator()(size_t row, size_t col) const noexcept { return m[row][col]; }
};

// StableNeoHookean (same as simplified_stable_neo_hookean_3d) for SoftVertexTriangleStitch tetrahedra.
// Generated from scripts/symbol_calculation/soft_vertex_triangle_stitch.ipynb; source: simplified_stable_neo_hookean_3d.inl

template <typename T>
__host__ __device__ void E(T& R, const T& mu, const T& lambda, const Vector9<T>& VecF)
{
/* Sub Exprs */
...
/* Simplified Expr */
R = (1.0/2.0)*lambda*luisa::pow(x0, 2) - mu*x0 + (1.0/2.0)*mu*(luisa::pow(VecF(0), 2) + luisa::pow(VecF(3), 2) + luisa::pow(VecF(6), 2) + luisa::pow(VecF(1), 2) + luisa::pow(VecF(4), 2) + luisa::pow(VecF(7), 2) + luisa::pow(VecF(2), 2) + luisa::pow(VecF(5), 2) + luisa::pow(VecF(8), 2) - 3) + luisa::pow(mu, 2)/luisa::pow(lambda, 2);
}

template <typename T>
__host__ __device__ void dEdVecF(Vector9<T>& R, const T& mu, const T& lambda, const Vector9<T>& VecF)
{
/* Sub Exprs */
...
template <typename T>
__host__ __device__ void ddEddVecF(Matrix9x9<T>& R, const T& mu, const T& lambda, const Vector9<T>& VecF)
{
/* Sub Exprs */
auto x0 = VecF(4)*VecF(8);
auto x1 = VecF(7)*VecF(5);
