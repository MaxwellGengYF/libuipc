...
{
template <>
class SimSystemCreator<luisa::GlobalContactManager>
{
  public:
    static U<luisa::GlobalContactManager> create(luisa::SimEngine& engine)
    {
        ...
        if(contact_enable)
            return make_unique<luisa::GlobalContactManager>(engine);
        return nullptr;
    }
};
}  // namespace uipc::backend
...
BufferView<IndexT> GlobalContactManager::contact_mask_tabular() const noexcept
{
    return m_impl.contact_mask_tabular.view();
}

BufferView<IndexT> GlobalContactManager::subscene_mask_tabular() const noexcept
{
    return m_impl.subscene_mask_tabular.view();
}
...
void GlobalContactManager::Impl::init(WorldVisitor& world)
{
    ...
    auto& engine = global_vertex_manager->engine();
    auto& device = engine.luisa_device();
    auto& stream = engine.compute_stream();

    auto vert_count = global_vertex_manager->positions().size();
    vert_is_active_contact = device.create_buffer<IndexT>(vert_count);
    vert_disp_norms = device.create_buffer<Float>(vert_count);
    max_disp_norm = device.create_buffer<Float>(1);

    // Initialize to 0
    stream << vert_is_active_contact.fill(0)
           << vert_disp_norms.fill(0.0f);
...
void GlobalContactManager::Impl::_build_contact_tabular(WorldVisitor& world)
{
    ...
    contact_tabular_width = N;
    contact_tabular = make_shared<Buffer<ContactCoeff>>(device.create_buffer<ContactCoeff>(N * N));
    stream << contact_tabular->copy_from(h_contact_tabular.data());

    contact_mask_tabular = device.create_buffer<IndexT>(N * N);
    stream << contact_mask_tabular.copy_from(h_contact_mask_tabular.data());
}
...
void GlobalContactManager::Impl::_build_subscene_tabular(WorldVisitor& world)
{
    ...
    subscene_mask_tabular = device.create_buffer<IndexT>(SN * SN);
    stream << subscene_mask_tabular.copy_from(h_subcene_mask_tabular.data());
}
...
Float GlobalContactManager::Impl::compute_cfl_condition()
{
    ...
    if(global_trajectory_filter)
    {
        ...
        // Kernel to compute displacement norms
        Kernel1D compute_disp_norms_kernel = [&](BufferVar<Vector3> disps,
                                                  BufferVar<Float> disp_norms,
                                                  BufferVar<IndexT> is_contact_active) noexcept
        {
            auto i = dispatch_id().x;
            $if(i < disps.size())
            {
                // if the contact is not active, then the displacement is ignored
                $if(is_contact_active.read(i) != 0)
                {
                    auto d = disps.read(i);
                    disp_norms.write(i, length(d));
                }
                $else
                {
                    disp_norms.write(i, 0.0f);
                };
            };
        };

        auto shader = device.compile(compute_disp_norms_kernel);
        stream << shader(displacements, vert_disp_norms, vert_is_active_contact)
                      .dispatch(displacements.size());

        // Use buffer reduction to find max
        // In LuisaCompute, we need to implement reduction differently
        // For now, copy to host and compute max
        vector<Float> h_disp_norms(displacements.size());
        stream << vert_disp_norms.copy_to(h_disp_norms.data())
               << synchronize();

        Float h_max_disp_norm = 0.0;
        for(auto& norm : h_disp_norms)
        {
            h_max_disp_norm = std::max(h_max_disp_norm, norm);
        }
...
BufferView<ContactCoeff> GlobalContactManager::AdaptiveParameterInfo::contact_tabular() const noexcept
{
    return m_impl->contact_tabular->view();
}

S<Buffer<ContactCoeff>> GlobalContactManager::AdaptiveParameterInfo::exchange_contact_tabular(
    S<Buffer<ContactCoeff>> new_buffer) const noexcept
{
...
BufferView<ContactCoeff> GlobalContactManager::contact_tabular() const noexcept
{
    return m_impl.contact_tabular->view();
}
