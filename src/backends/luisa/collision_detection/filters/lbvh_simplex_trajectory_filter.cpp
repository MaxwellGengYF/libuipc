...
void LBVHSimplexTrajectoryFilter::Impl::detect(DetectInfo& info, WorldVisitor& world)
{
    ...
    // Build LBVH structures
    if(!m_impl.lbvh_E)
        m_impl.lbvh_E = std::make_unique<Impl::ThisBVH>(device, stream);
    if(!m_impl.lbvh_T)
        m_impl.lbvh_T = std::make_unique<Impl::ThisBVH>(device, stream);

    m_impl.lbvh_E->build(edge_aabbs.view());
    m_impl.lbvh_T->build(triangle_aabbs.view());

    // Compute table dimensions for mask tables
    SizeT contact_table_size = info.contact_mask_tabular().size();
    SizeT contact_table_dim = static_cast<SizeT>(std::sqrt(static_cast<double>(contact_table_size)));
    SizeT subscene_table_size = info.subscene_mask_tabular().size();
    ...
    if(codimVs.size() > 0)
    {
        ...
        {
            if(!m_impl.lbvh_CodimP)
                m_impl.lbvh_CodimP = std::make_unique<Impl::ThisBVH>(device, stream);
            m_impl.lbvh_CodimP->build(codim_point_aabbs.view());
            ...
            auto dxs_view = dxs;
            auto thicknesses_view = info.thicknesses();
            auto dimensions_view = info.dimensions();
            ...
            auto v2b_view = info.v2b();
            auto body_self_collision_view = info.body_self_collision();
            auto d_hats_view = info.d_hats();

            m_impl.lbvh_CodimP->query(
                point_aabbs.view(),
                [&](IndexT i, IndexT j) -> bool
                {
                    // This predicate runs on device, need to capture by value
                    // We need to use a kernel-based approach
                    return true;  // Placeholder - actual filtering happens in kernel
                },
                m_impl.candidate_AllP_CodimP_pairs);
        }
        ...
        if(codimVs.size())
        {
            ...
            m_impl.lbvh_E->query(
                ...
                m_impl.candidate_CodimP_AllE_pairs);
        }
    }
    ...
    // Use AllE to query AllE
    {
        ...
        m_impl.lbvh_E->detect(
            [&](IndexT i, IndexT j) -> bool { return true; },
            m_impl.candidate_AllE_AllE_pairs);
    }
    ...
    // Use AllP to query AllT
    {
        ...
        m_impl.lbvh_T->query(
            point_aabbs.view(),
            [&](IndexT i, IndexT j) -> bool { return true; },
            m_impl.candidate_AllP_AllT_pairs);
    }
}

void LBVHSimplexTrajectoryFilter::Impl::filter_active(FilterActiveInfo& info, WorldVisitor& world)
{
    auto& engine = static_cast<SimEngine&>(world.sim_engine());
    auto& device = engine.device();
    auto  stream = engine.compute_stream();
    ...
    if(N_PCoimP > 0)
    {
                                        ...
                                        BufferVar<Float> d_hats) noexcept
        {
            ...
            $if(i < N_PCoimP)
            {
                ...
                // Note: MUDA_ASSERT not available in LuisaCompute
...
void LBVHSimplexTrajectoryFilter::Impl::filter_toi(FilterTOIInfo& info, WorldVisitor& world)
{
    auto& engine = static_cast<SimEngine&>(world.sim_engine());
    auto& device = engine.device();
    auto  stream = engine.compute_stream();
