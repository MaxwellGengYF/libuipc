...
luisa::compute::BufferView<const IndexT> GlobalBodyManager::coindices() const noexcept
{
    return m_impl.coindices.view();
}

luisa::compute::BufferView<const IndexT> GlobalBodyManager::self_collision() const noexcept
{
    return m_impl.self_collision.view();
}

luisa::compute::BufferView<IndexT> GlobalBodyManager::BodyAttributeInfo::coindices() const noexcept
{
    return m_impl->subview(m_impl->coindices, m_index);
}

luisa::compute::BufferView<IndexT> GlobalBodyManager::BodyAttributeInfo::self_collision() const noexcept
{
