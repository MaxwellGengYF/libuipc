...
void AffineBodyStateAccessorFeatureOverrider::do_copy_from(const geometry::SimplicialComplex& state_geo)
{
    ...
    auto& stream = m_abd.engine().compute_stream();
    ...
    if(trans)
    {
        ...
        stream << q_subview.copy_from(m_buffer.data())
    ...
    if(vel)
    {
        ...
        stream << q_v_subview.copy_from(m_buffer.data())
               << luisa::compute::synchronize();
    }
...
void AffineBodyStateAccessorFeatureOverrider::do_copy_to(geometry::SimplicialComplex& state_geo)
{
    ...
    if(trans)
    {
        ...
        stream << q_subview.copy_to(m_buffer.data())
               << luisa::compute::synchronize();
    ...
    if(vel)
    {
        ...
        stream << q_v_subview.copy_to(m_buffer.data())
               << luisa::compute::synchronize();
