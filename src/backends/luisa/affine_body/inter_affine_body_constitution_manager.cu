...
{
template <>
class backend::SimSystemCreator<luisa::InterAffineBodyConstitutionManager>
{
  public:
    static U<luisa::InterAffineBodyConstitutionManager> create(SimEngine& engine)
    {
        ...
        return uipc::make_unique<luisa::InterAffineBodyConstitutionManager>(engine);
    }
};
}  // namespace uipc::backend
...
luisa::compute::BufferView<Float> InterAffineBodyConstitutionManager::EnergyInfo::energies() const noexcept
{
    auto [offset, count] = m_impl->constitution_energy_offsets_counts[m_index];
    return m_energies.subview(offset, count);
}

ABDLinearSubsystem::DoubletVectorView<Float, 12> InterAffineBodyConstitutionManager::GradientHessianInfo::gradients() const noexcept
{
    auto [offset, count] = m_impl->constitution_gradient_offsets_counts[m_index];
    ABDLinearSubsystem::DoubletVectorView<Float, 12> result;
    result.indices = m_gradients.indices.subview(offset, count);
    result.values = m_gradients.values.subview(offset, count);
    result.count = count;
    return result;
}

ABDLinearSubsystem::TripletMatrixView<Float, 12, 12> InterAffineBodyConstitutionManager::GradientHessianInfo::hessians() const noexcept
{
    auto [offset, count] = m_impl->constitution_hessian_offsets_counts[m_index];
    ABDLinearSubsystem::TripletMatrixView<Float, 12, 12> result;
    result.row_indices = m_hessians.row_indices.subview(offset, count);
    result.col_indices = m_hessians.col_indices.subview(offset, count);
    result.values = m_hessians.values.subview(offset, count);
    result.count = count;
    return result;
}
...
luisa::compute::BufferView<const Vector12> InterAffineBodyConstitutionManager::BaseInfo::qs() const noexcept
{
    return m_impl->affine_body_dynamics->m_impl.body_id_to_q.view();
}

luisa::compute::BufferView<const Vector12> InterAffineBodyConstitutionManager::BaseInfo::q_prevs() const noexcept
{
    return m_impl->affine_body_dynamics->m_impl.body_id_to_q_prev.view();
}

luisa::compute::BufferView<const ABDJacobiDyadicMass> InterAffineBodyConstitutionManager::BaseInfo::body_masses() const noexcept
{
    return m_impl->affine_body_dynamics->m_impl.body_id_to_abd_mass.view();
}

luisa::compute::BufferView<const IndexT> InterAffineBodyConstitutionManager::BaseInfo::is_fixed() const noexcept
{
