...
class ExternalArticulationConstraint final : public InterAffineBodyConstraint
{
  public:
    ...
    void do_init(InterAffineBodyAnimator::FilteredInfo& info) override
    {
        ...
        art_id_to_joint_offsets = device.create_buffer<IndexT>(h_art_joint_offsets.size());
        ...
        art_id_to_joint_counts = device.create_buffer<IndexT>(h_art_joint_counts.size());
        art_id_to_joint_counts.view().copy_from(h_art_joint_counts.data());

        joint_id_to_art_id = device.create_buffer<IndexT>(n_joint);
        joint_id_to_art_id.view().copy_from(h_joint_id_to_art_id.data());

        joint_id_to_uid = device.create_buffer<U64>(h_joint_id_to_uid.size());
        joint_id_to_uid.view().copy_from(h_joint_id_to_uid.data());

        joint_id_to_body_ids = device.create_buffer<Vector2i>(h_joint_id_to_body_ids.size());
        joint_id_to_body_ids.view().copy_from(h_joint_id_to_body_ids.data());

        // init delta_theta to zero
        joint_id_to_delta_theta = device.create_buffer<Float>(n_joint);
        joint_id_to_delta_theta.view().fill(0.0f);
        h_joint_id_to_delta_theta.resize(n_joint, 0.0f);

        // init delta_theta_tilde
        joint_id_to_delta_theta_tilde = device.create_buffer<Float>(h_joint_id_to_delta_theta_tilde.size());
        joint_id_to_delta_theta_tilde.view().copy_from(h_joint_id_to_delta_theta_tilde.data());

        joint_id_to_L_basis = device.create_buffer<Vector6>(h_joint_id_to_L_basis.size());
        joint_id_to_L_basis.view().copy_from(h_joint_id_to_L_basis.data());

        joint_id_to_R_basis = device.create_buffer<Vector6>(h_joint_id_to_R_basis.size());
        joint_id_to_R_basis.view().copy_from(h_joint_id_to_R_basis.data());

        joint_id_to_G_theta = device.create_buffer<Float>(n_joint);

        // Joint-Joint Info
        auto h_art_mass_offsets = h_art_id_to_joint_joint_offsets_counts.offsets();
        art_id_to_joint_joint_offsets = device.create_buffer<IndexT>(h_art_mass_offsets.size());
        ...
        art_id_to_joint_joint_counts = device.create_buffer<IndexT>(h_art_mass_counts.size());
        art_id_to_joint_joint_counts.view().copy_from(h_art_mass_counts.data());

        joint_joint_id_to_mass = device.create_buffer<Float>(h_joint_joint_id_to_mass.size());
        joint_joint_id_to_mass.view().copy_from(h_joint_joint_id_to_mass.data());

        joint_joint_id_to_joint_ij = device.create_buffer<Vector2i>(h_joint_joint_id_to_joint_ij.size());
        joint_joint_id_to_joint_ij.view().copy_from(h_joint_joint_id_to_joint_ij.data());

        // Ref q_prev
        joint_id_to_ref_q_prev_ids = device.create_buffer<Vector2i>(h_joint_id_to_ref_q_prev_ids.size());
        joint_id_to_ref_q_prev_ids.view().copy_from(h_joint_id_to_ref_q_prev_ids.data());

        ref_q_prevs = device.create_buffer<Vector12>(h_ref_q_prevs.size());
    ...
    void do_compute_energy(InterAffineBodyAnimator::ComputeEnergyInfo& info) override
    {
        ...
        SizeT joint_joint_count = joint_joint_id_to_mass.size();

        Kernel1D compute_energy_kernel = [&](BufferVar<Vector2i> joint_joint_id_to_joint_ij,
                                              BufferVar<Float> joint_joint_id_to_mass,
                                              BufferVar<IndexT> art_id_to_joint_joint_offsets,
                                              BufferVar<IndexT> art_id_to_joint_joint_counts,
                                              BufferVar<IndexT> art_id_to_joint_offsets,
                                              BufferVar<IndexT> art_id_to_joint_counts,
                                              BufferVar<Float> joint_id_to_delta_theta,
                                              BufferVar<Float> joint_id_to_delta_theta_tilde,
                                              BufferVar<Vector2i> joint_id_to_body_ids,
                                              BufferVar<Vector6> joint_id_to_L_basis,
                                              BufferVar<Vector6> joint_id_to_R_basis,
                                              BufferVar<U64> joint_id_to_uid,
                                              BufferVar<Vector12> qs,
                                              BufferVar<Vector12> q_prevs,
                                              BufferVar<Vector2i> joint_id_to_ref_q_prev_ids,
                                              BufferVar<Vector12> ref_q_prevs,
                                              BufferVar<Float> energies) {
            auto I = dispatch_id().x;
            $if(I < joint_joint_count) {
                Float m_ij = joint_joint_id_to_mass.read(I);
                Vector2i ij = joint_joint_id_to_joint_ij.read(I);

                auto compute_delta_theta = [&](IndexT joint_id) -> Float {
                    Vector6 basis_k = joint_id_to_L_basis.read(joint_id);
                    Vector6 basis_l = joint_id_to_R_basis.read(joint_id);
                    Vector2i body_ids = joint_id_to_body_ids.read(joint_id);

                    Vector12 qk = qs.read(body_ids[0]);
                    Vector12 ql = qs.read(body_ids[1]);

                    Vector2i ref_q_prev_ids = joint_id_to_ref_q_prev_ids.read(joint_id);

                    Vector12 q_prevk = ref_q_prev_ids[0] >= 0 ?
                                       ref_q_prevs.read(ref_q_prev_ids[0]) :
                                       q_prevs.read(body_ids[0]);

                    Vector12 q_prevl = ref_q_prev_ids[1] >= 0 ?
                                       ref_q_prevs.read(ref_q_prev_ids[1]) :
                                       q_prevs.read(body_ids[1]);

                    U64 joint_uid = joint_id_to_uid.read(joint_id);

                    Float delta_theta;

                    if(joint_uid == ExternalArticulationConstituion::RevoluteJointConstitutionUID)
                    {
                        ERJ::DeltaTheta(delta_theta, basis_k, qk, q_prevk, basis_l, ql, q_prevl);
                    }
                    else if(joint_uid == ExternalArticulationConstituion::PrismaticJointConstitutionUID)
                    {
                ...
                Float delta_theta_tilde_i = joint_id_to_delta_theta_tilde.read(ij[0]);
                Float delta_theta_tilde_j = joint_id_to_delta_theta_tilde.read(ij[1]);

                Float energy = 0.5f * (delta_theta_i - delta_theta_tilde_i)
                               * m_ij * (delta_theta_j - delta_theta_tilde_j);
                energies.write(I, energy);
            };
        };

        auto kernel = device.compile(compute_energy_kernel);
        stream << kernel(joint_joint_id_to_joint_ij.view(),
                         joint_joint_id_to_mass.view(),
                         art_id_to_joint_joint_offsets.view(),
                         art_id_to_joint_joint_counts.view(),
                         art_id_to_joint_offsets.view(),
                         art_id_to_joint_counts.view(),
                         joint_id_to_delta_theta.view(),
                         joint_id_to_delta_theta_tilde.view(),
                         joint_id_to_body_ids.view(),
                         joint_id_to_L_basis.view(),
                         joint_id_to_R_basis.view(),
                         joint_id_to_uid.view(),
                         qs_view,
                         q_prevs_view,
                         joint_id_to_ref_q_prev_ids.view(),
                         ref_q_prevs.view(),
                         energies_view)
                      .dispatch(joint_joint_count);
    }
    ...
    void do_compute_gradient_hessian(InterAffineBodyAnimator::GradientHessianInfo& info) override
    {
        ...
        SizeT joint_joint_count = joint_joint_id_to_mass.size();
        SizeT joint_count = joint_id_to_art_id.size();

        // Compute G^theta using kernel
        Kernel1D compute_G_theta_kernel = [&](BufferVar<Vector2i> joint_joint_id_to_joint_ij,
                                               BufferVar<Float> joint_joint_id_to_mass,
                                               BufferVar<Vector6> joint_id_to_L_basis,
                                               BufferVar<Vector6> joint_id_to_R_basis,
                                               BufferVar<Vector2i> joint_id_to_body_ids,
                                               ...
                                               BufferVar<Vector12> qs,
                                               BufferVar<Vector12> q_prevs,
                                               BufferVar<Float> joint_id_to_G_theta) {
            auto I = dispatch_id().x;
            $if(I < joint_joint_count) {
                Vector2i ij = joint_joint_id_to_joint_ij.read(I);
                Float m_ij = joint_joint_id_to_mass.read(I);

                IndexT joint_id = ij[1];
                ...
                Float delta_theta_tilde_j = joint_id_to_delta_theta_tilde.read(joint_id);

                Float G_theta = m_ij * (delta_theta_j - delta_theta_tilde_j);
                
                // Atomic add to accumulate G_theta
                joint_id_to_G_theta.atomic(joint_id).fetch_add(G_theta);
            };
        };

        // Reset G_theta to zero first
        stream << joint_id_to_G_theta.view().fill(0.0f);

        auto G_theta_kernel = device.compile(compute_G_theta_kernel);
        stream << G_theta_kernel(joint_joint_id_to_joint_ij.view(),
                                 joint_joint_id_to_mass.view(),
                                 joint_id_to_L_basis.view(),
                                 joint_id_to_R_basis.view(),
                                 joint_id_to_body_ids.view(),
        ...
        // Compute Gradient
        Kernel1D compute_gradient_kernel = [&](BufferVar<Float> joint_id_to_G_theta,
                                                BufferVar<Vector2i> joint_id_to_body_ids,
                                                BufferVar<Vector6> joint_id_to_L_basis,
                                                BufferVar<Vector6> joint_id_to_R_basis,
                                                BufferVar<Vector2i> joint_id_to_ref_q_prev_ids,
            ...
            auto I = dispatch_id().x;
            $if(I < joint_count) {
                Float G_theta = joint_id_to_G_theta.read(I);
                Vector2i body_ids = joint_id_to_body_ids.read(I);
                Vector6 basis_k = joint_id_to_L_basis.read(I);
                Vector6 basis_l = joint_id_to_R_basis.read(I);

                Vector12 qk = qs.read(body_ids[0]);
                Vector12 ql = qs.read(body_ids[1]);

                Vector2i ref_q_prev_ids = joint_id_to_ref_q_prev_ids.read(I);
                ...
                U64 joint_uid = joint_id_to_uid.read(I);
                ...
                {
                    G24 = Vector<Float, 24>::Zero();
                }

                G24 *= G_theta;

                // Write gradient for body 0 (first 12 components)
                gradient_indices.write(I * 2 + 0, body_ids[0]);
                Vector12 G_i;
                for(int k = 0; k < 12; ++k) {
                    G_i[k] = G24[k];
                }
                gradients.write(I * 2 + 0, G_i);

                // Write gradient for body 1 (last 12 components)
                gradient_indices.write(I * 2 + 1, body_ids[1]);
                Vector12 G_j;
                for(int k = 0; k < 12; ++k) {
                    G_j[k] = G24[12 + k];
                }
                gradients.write(I * 2 + 1, G_j);
            };
        };

        auto grad_kernel = device.compile(compute_gradient_kernel);
        stream << grad_kernel(joint_id_to_G_theta.view(),
                              joint_id_to_body_ids.view(),
                              joint_id_to_L_basis.view(),
                              joint_id_to_R_basis.view(),
                              joint_id_to_ref_q_prev_ids.view(),
        ...
        // Compute Hessian
        Kernel1D compute_hessian_kernel = [&](BufferVar<Float> joint_id_to_G_theta,
                                               BufferVar<Vector2i> joint_id_to_body_ids,
                                               BufferVar<Vector6> joint_id_to_L_basis,
                                               BufferVar<Vector6> joint_id_to_R_basis,
                                               BufferVar<Vector2i> joint_joint_id_to_joint_ij,
                                               BufferVar<Float> joint_joint_id_to_mass,
                                               BufferVar<Vector2i> joint_id_to_ref_q_prev_ids,
                                               BufferVar<Vector12> ref_q_prevs,
                                               BufferVar<U64> joint_id_to_uid,
            ...
            $if(I < joint_joint_count) {
                ...
                auto compute_dDeltaTheta_dQ = [&](IndexT joint_id) -> Vector<Float, 24> {
                    ...
                    U64 uid = joint_id_to_uid.read(joint_id);
                    if(uid == ExternalArticulationConstituion::RevoluteJointConstitutionUID)
                    {
                        ERJ::dDeltaTheta_dQ(dDeltaTheta_dQ, basis_k, qk, q_prevk, basis_l, ql, q_prevl);
                    }
                    else if(uid == ExternalArticulationConstituion::PrismaticJointConstitutionUID)
                    {
                        EPJ::dDeltaTheta_dQ(dDeltaTheta_dQ, basis_k, qk, q_prevk, basis_l, ql, q_prevl);
                    }
                    else
                    {
                        dDeltaTheta_dQ = Vector<Float, 24>::Zero();
                    }
                ...
                // H24x24 = dDeltaTheta_dQ_i * m_ij * dDeltaTheta_dQ_j^T
                Matrix<Float, 24, 24> H24x24;
                for(int r = 0; r < 24; ++r) {
                    for(int c = 0; c < 24; ++c) {
                        H24x24(r, c) = dDeltaTheta_dQ_i[r] * m_ij * dDeltaTheta_dQ_j[c];
                    }
                }
                ...
                $if(ij[0] == ij[1]) {
                    ...
                    Float G_theta_i = joint_id_to_G_theta.read(i);
                ...
                Vector2i body_ids_i = joint_id_to_body_ids.read(ij[0]);
                Vector2i body_ids_j = joint_id_to_body_ids.read(ij[1]);

                // Write hessian blocks
                // For half-hessian storage, we need to handle upper triangular
                IndexT row_L, col_R;
                $if(body_ids_i[0] < body_ids_j[0]) {
                    row_L = body_ids_i[0];
                    col_R = body_ids_j[0];
                }
                $else {
                ...
                hessian_row_indices.write(I, row_L);
                hessian_col_indices.write(I, col_R);

                // Extract 12x12 block from H24x24
                Matrix12x12 H12x12;
                for(int r = 0; r < 12; ++r) {
                    for(int c = 0; c < 12; ++c) {
                        H12x12(r, c) = H24x24(r, c);
                    }
                }
                hessians.write(I, H12x12);
            };
        };

        auto hess_kernel = device.compile(compute_hessian_kernel);
        stream << hess_kernel(joint_id_to_G_theta.view(),
                              joint_id_to_body_ids.view(),
                              joint_id_to_L_basis.view(),
                              joint_id_to_R_basis.view(),
                              joint_joint_id_to_joint_ij.view(),
                              joint_joint_id_to_mass.view(),
                              joint_id_to_ref_q_prev_ids.view(),
                              ref_q_prevs.view(),
                              joint_id_to_uid.view(),
    ...
    void write_scene()
    {
        ...
        luisa::vector<Float> h_delta_theta(joint_id_to_delta_theta.size());
        joint_id_to_delta_theta.view().copy_to(h_delta_theta.data());
        ...
        this->for_each(
                ...
                geometry::Geometry&                                    geo)
            {
                ...
                if(delta_theta)
                {
                    ...
                    std::ranges::copy(span{h_delta_theta}.subspan(offset, count),
...
class ExternalArticulationConstraintTimeIntegrator final : public ABDTimeIntegrator
{
  public:
    ...
    void do_update_state(UpdateVelocityInfo& info) override
    {
        ...
        auto qs_view = abd->qs();
        auto q_prevs_view = abd->q_prevs();
        auto joint_id_to_delta_theta_view = c->joint_id_to_delta_theta.view();

        SizeT joint_count = c->joint_id_to_delta_theta.size();

        Kernel1D update_delta_theta_kernel = [&](BufferVar<Float> joint_id_to_delta_theta,
                                                  BufferVar<Vector2i> joint_id_to_body_ids,
                                                  BufferVar<Vector6> joint_id_to_L_basis,
                                                  BufferVar<Vector6> joint_id_to_R_basis,
                                                  BufferVar<Vector12> qs,
            ...
            auto I = dispatch_id().x;
            $if(I < joint_count) {
                Vector6 L_basis = joint_id_to_L_basis.read(I);
                Vector6 R_basis = joint_id_to_R_basis.read(I);
                Vector2i body_ids = joint_id_to_body_ids.read(I);
                ...
                joint_id_to_delta_theta.write(I, delta_theta);
            };
        };

        auto kernel = device.compile(update_delta_theta_kernel);
        stream << kernel(joint_id_to_delta_theta_view,
                         c->joint_id_to_body_ids.view(),
                         c->joint_id_to_L_basis.view(),
                         c->joint_id_to_R_basis.view(),
                         qs_view,
                         q_prevs_view,
                         c->joint_id_to_ref_q_prev_ids.view(),
                         c->ref_q_prevs.view(),
                         c->joint_id_to_uid.view())
                      .dispatch(joint_count);
    }
};
