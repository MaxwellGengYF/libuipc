...
{
template <>
class backend::SimSystemCreator<luisa::AffineBodyDynamics>
{
  public:
    static U<luisa::AffineBodyDynamics> create(SimEngine& engine)
    {
        ...
        return uipc::make_unique<luisa::AffineBodyDynamics>(engine);
    }
};
}  // namespace uipc::backend
...
void AffineBodyDynamics::Impl::_build_geometry_on_device(WorldVisitor& world)
{
    ...
    // Helper lambda to create buffer and copy from host
    auto create_buffer_and_copy = [&device, &stream]<typename T>(span<T> src) -> Buffer<T>
    {
        Buffer<T> buf = device.create_buffer<T>(src.size());
        stream << buf.view().copy_from(src.data());
        return buf;
    };

    body_id_to_q = create_buffer_and_copy(span{h_body_id_to_q});
    body_id_to_q_v = create_buffer_and_copy(span{h_body_id_to_q_v});
    body_id_to_dim = create_buffer_and_copy(span{h_body_id_to_dim});
    vertex_id_to_J = create_buffer_and_copy(span{h_vertex_id_to_J});
    vertex_id_to_body_id = create_buffer_and_copy(span{h_vertex_id_to_body_id});
    body_id_to_abd_mass = create_buffer_and_copy(span{h_body_id_to_abd_mass});
    body_id_to_volume = create_buffer_and_copy(span{h_body_id_to_volume});
    body_id_to_abd_mass_inv = create_buffer_and_copy(span{h_body_id_to_abd_mass_inv});
    body_id_to_abd_gravity = create_buffer_and_copy(span{h_body_id_to_abd_gravity});
    body_id_to_is_fixed = create_buffer_and_copy(span{h_body_id_to_is_fixed});
    body_id_to_is_dynamic = create_buffer_and_copy(span{h_body_id_to_is_dynamic});
    body_id_to_external_kinetic = create_buffer_and_copy(span{h_body_id_to_external_kinetic});

    // Copy q to temp buffers
    body_id_to_q_temp = device.create_buffer<Vector12>(abd_body_count);
    body_id_to_q_tilde = device.create_buffer<Vector12>(abd_body_count);
    body_id_to_q_prev = device.create_buffer<Vector12>(abd_body_count);
    
    stream << body_id_to_q_temp.copy_from(body_id_to_q.view())
           << body_id_to_q_tilde.copy_from(body_id_to_q.view())
           << body_id_to_q_prev.copy_from(body_id_to_q.view());

    // Create and zero-initialize dq buffer
    body_id_to_dq = device.create_buffer<Vector12>(abd_body_count);
    
    // Zero-fill the dq buffer using a kernel or fill operation
    // For now, we'll copy from a zeroed host vector
    vector<Vector12> zero_dq(abd_body_count, Vector12::Zero());
    stream << body_id_to_dq.view().copy_from(zero_dq.data());
...
void AffineBodyDynamics::Impl::_download_geometry_to_host()
{
    ...
    stream << body_id_to_q.view().copy_to(h_body_id_to_q.data())
...
bool AffineBodyDynamics::Impl::dump(DumpInfo& info)
{
    ...
    return dump_q.dump(fmt::format("{}q.{}", path, frame), body_id_to_q, stream)
           && dump_q_v.dump(fmt::format("{}q_v.{}", path, frame), body_id_to_q_v, stream)
           && dump_q_prev.dump(fmt::format("{}q_prev.{}", path, frame), body_id_to_q_prev, stream);
}
...
{
    auto& stream = engine().compute_stream();
    dump_q.apply_to(body_id_to_q, stream);
    dump_q_v.apply_to(body_id_to_q_v, stream);
    dump_q_prev.apply_to(body_id_to_q_prev, stream);
}
