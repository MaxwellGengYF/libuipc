...
luisa::compute::BufferView<const Vector12> AffineBodyConstitution::BaseInfo::qs() const noexcept
{
    auto& abd = m_impl->abd();
    return abd.subview(abd.body_id_to_q, m_index);
}

luisa::compute::BufferView<const Vector12> AffineBodyConstitution::BaseInfo::q_prevs() const noexcept
{
    auto& abd = m_impl->abd();
    return abd.subview(abd.body_id_to_q_prev, m_index);
}

luisa::compute::BufferView<const Float> AffineBodyConstitution::BaseInfo::volumes() const noexcept
{
    auto& abd = m_impl->abd();
