...
void GlobalLinearSystem::do_build()
{
    ...
    // Initialize spmver with the stream
    m_impl.spmver.set_stream(engine().stream());
}
...
void GlobalLinearSystem::_dump_A_b()
{
    ...
    auto output_path_A = fmt::format("{}A.{}.{}.mtx",
                                     ...
                                     engine().frame(),
                                     engine().newton_iter());
    export_matrix_market(output_path_A, m_impl.bcoo_A.view(), engine().stream());
    ...
    auto output_path_b = fmt::format("{}b.{}.{}.mtx",
                                     ...
                                     engine().frame(),
                                     engine().newton_iter());
    export_vector_market(output_path_b, m_impl.b.view(), engine().stream());
...
bool GlobalLinearSystem::Impl::_update_subsystem_extent()
{
    ...
    if(x.size() < total_dof)
    {
        auto reserve_count = static_cast<size_t>(total_dof * reserve_ratio);
        x = device.create_buffer<Float>(reserve_count);
        b = device.create_buffer<Float>(reserve_count);
    }
    auto blocked_dof = total_dof / DoFBlockSize;
    triplet_A.reshape(blocked_dof, blocked_dof);
    // Note: x and b don't have resize(), we track size separately
    // The actual size is stored implicitly in diag_dof_offsets_counts
    ...
    // Check if we need to resize triplet_A buffers
    if(triplet_A.capacity < total_triplet)
    {
        auto reserve_count = static_cast<size_t>(total_triplet * reserve_ratio);
        triplet_A.row_indices = device.create_buffer<int>(reserve_count);
        triplet_A.col_indices = device.create_buffer<int>(reserve_count);
        triplet_A.values = device.create_buffer<Eigen::Matrix<Float, 3, 3>>(reserve_count);
        triplet_A.capacity = reserve_count;
        
        bcoo_A.block_row_indices = device.create_buffer<int>(reserve_count);
        bcoo_A.block_col_indices = device.create_buffer<int>(reserve_count);
        bcoo_A.block_values = device.create_buffer<Eigen::Matrix<Float, 3, 3>>(reserve_count);
        bcoo_A.block_capacity = reserve_count;
    }
    triplet_A.count = total_triplet;
...
void GlobalLinearSystem::Impl::_assemble_linear_system()
{
    ...
    // Fill values with zero matrices
    stream << triplet_A.values.view().fill(Eigen::Matrix<Float, 3, 3>::Zero())
           << triplet_A.col_indices.view().fill(-1)
           << triplet_A.row_indices.view().fill(-1);
...
void GlobalLinearSystem::Impl::apply_preconditioner(luisa::compute::BufferView<Float>       z,
                                                    luisa::compute::BufferView<const Float> r)
{
    ...
    if(!global_preconditioner)
    {
        ...
        for(auto i : no_precond_diag_subsystem_indices)
        {
            ...
            engine().stream() << z_sub.copy_from(r_sub);
        }
    }
}

void GlobalLinearSystem::Impl::spmv(Float                                  a,
                                    luisa::compute::BufferView<const Float> x_view,
                                    Float                                  b_scalar,
                                    luisa::compute::BufferView<Float>       y)
{
    spmver.rbk_sym_spmv(a, bcoo_A.view(), x_view, b_scalar, y);

    // Just some debug options
    //  * spmver.sym_spmv(a, bcoo_A.view(), x_view, b_scalar, y);      // Slightly slower
    //  * spmver.cpu_sym_spmv(a, bcoo_A.view(), x_view, b_scalar, y);  // Much slower
}

void GlobalLinearSystem::Impl::spmv_dot(luisa::compute::BufferView<const Float> x_view,
                                        luisa::compute::BufferView<Float>       y,
                                        luisa::compute::BufferView<Float>       d_dot)
{
    spmver.rbk_sym_spmv_dot(1.0, bcoo_A.view(), x_view, 0.0, y, d_dot);
}

bool GlobalLinearSystem::Impl::accuracy_statisfied(luisa::compute::BufferView<Float> r)
{
    auto diag_dof_counts  = diag_dof_offsets_counts.counts();
    auto diag_dof_offsets = diag_dof_offsets_counts.offsets();
    ...
    for(auto&& [i, diag_subsystem] : enumerate(diag_subsystems.view()))
    {
        ...
        diag_subsystem->accuracy_check(info);
...
void GlobalLinearSystem::Impl::compute_gradient(ComputeGradientInfo& info)
{
    ...
    for(auto&& [i, diag_subsystem] : enumerate(diag_subsystem_view))
    {
        ...
        diag_info.m_hessians      = MutableTripletMatrix3{};  // Empty Hessian View
        diag_info.m_gradient_only = true;
        diag_info.m_component_flags = info.m_flags;
...
auto GlobalLinearSystem::AssemblyInfo::A() const -> CBCOOMatrixView3
{
    return m_impl->bcoo_A.view();
}
...
namespace uipc::backend::luisa
{
void GlobalLinearSystem::ComputeGradientInfo::buffer_view(luisa::compute::BufferView<Float> grad) noexcept
{
    m_gradients = grad;
}
