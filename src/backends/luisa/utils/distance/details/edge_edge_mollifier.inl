namespace uipc::backend::luisa::distance::details {
...
LC_GPU_CALLABLE void edge_edge_cross_norm2(const luisa::Vector<T, 3>& ea0,
                                           ...
                                           T&                         result)
{
    luisa::Vector<T, 3> ea = ea1 - ea0;
    luisa::Vector<T, 3> eb = eb1 - eb0;
    luisa::Vector<T, 3> cross_prod = luisa::cross(ea, eb);
    result = luisa::dot(cross_prod, cross_prod);
}
                                                 ...
                                                 luisa::Matrix<T, 12, 12>&  H)
{
    ...
    if(EECrossSqNorm < eps_x)
    {
        ...
        H *= q_g;
        H += q_H * luisa::Matrix<T, 12, 12>::outer_product(g, g);
    }
    else
    {
        H = luisa::Matrix<T, 12, 12>(T(0));
    }
}
...
LC_GPU_CALLABLE void edge_edge_mollifier_threshold(const luisa::Vector<T, 3>& ea0_rest,
                                                   ...
                                                   T&                         eps_x)
{
    luisa::Vector<T, 3> ea_rest = ea1_rest - ea0_rest;
    luisa::Vector<T, 3> eb_rest = eb1_rest - eb0_rest;
    eps_x = coeff * luisa::dot(ea_rest, ea_rest) * luisa::dot(eb_rest, eb_rest);
}
