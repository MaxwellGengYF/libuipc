...
#include <backends/luisa/utils/dump_utils.h>
  ...
  public:
    ...
    class Impl
    {
      public:
        ...
        luisa::compute::Buffer<IndexT> codim_0ds;

        luisa::compute::Buffer<Vector2i> codim_1ds;
        luisa::compute::Buffer<Float>    rest_lengths;

        luisa::compute::Buffer<Vector3i> codim_2ds;
        luisa::compute::Buffer<Float>    rest_areas;

        luisa::compute::Buffer<Vector4i> tets;
        luisa::compute::Buffer<Float>    rest_volumes;


        // Vertex Attributes:

        luisa::compute::Buffer<IndexT>  is_fixed;    // Vertex IsFixed
        luisa::compute::Buffer<IndexT>  is_dynamic;  // Vertex IsDynamic
        luisa::compute::Buffer<Vector3> gravities;   // Vertex Gravity

        luisa::compute::Buffer<Vector3> x_bars;    // Rest Positions
        luisa::compute::Buffer<Vector3> xs;        // Positions
        luisa::compute::Buffer<Vector3> dxs;       // Displacements
        luisa::compute::Buffer<Vector3> x_temps;   // Safe Positions for line search
        luisa::compute::Buffer<Vector3> vs;        // Velocities
        luisa::compute::Buffer<Vector3> x_tildes;  // Predicted Positions
        luisa::compute::Buffer<Vector3> x_prevs;   // Positions at last frame
        luisa::compute::Buffer<Float>   masses;    // Mass
        luisa::compute::Buffer<Float>   thicknesses;  // Thickness
        ...
        // \end{bmatrix}^{-1}
        // $$
        luisa::compute::Buffer<Matrix3x3> Dm3x3_invs;
