...
{
template <>
class backend::SimSystemCreator<luisa::FiniteElementMethod>
{
  public:
    static U<luisa::FiniteElementMethod> create(SimEngine& engine)
    {
        ...
        return uipc::make_unique<luisa::FiniteElementMethod>(engine);
    }
};
}  // namespace uipc::backend


bool operator<(const uipc::backend::luisa::FiniteElementMethod::DimUID& a,
               const uipc::backend::luisa::FiniteElementMethod::DimUID& b)
{
...
void FiniteElementMethod::Impl::_build_on_device()
{
    ...
    // 1) Vertex States
    xs = device.create_buffer<Vector3>(h_positions.size());
    stream << xs.copy_from(h_positions.data());

    x_bars = device.create_buffer<Vector3>(h_rest_positions.size());
    stream << x_bars.copy_from(h_rest_positions.data());

    x_temps  = device.create_buffer<Vector3>(xs.size());
    x_tildes = device.create_buffer<Vector3>(xs.size());
    x_prevs  = device.create_buffer<Vector3>(xs.size());
    stream << x_temps.copy_from(xs);
    stream << x_tildes.copy_from(xs);
    stream << x_prevs.copy_from(xs);

    is_fixed = device.create_buffer<IndexT>(h_vertex_is_fixed.size());
    stream << is_fixed.copy_from(h_vertex_is_fixed.data());

    is_dynamic = device.create_buffer<IndexT>(h_vertex_is_dynamic.size());
    stream << is_dynamic.copy_from(h_vertex_is_dynamic.data());

    gravities = device.create_buffer<Vector3>(h_gravities.size());
    stream << gravities.copy_from(h_gravities.data());

    dxs = device.create_buffer<Vector3>(xs.size());
    vs = device.create_buffer<Vector3>(h_velocities.size());
    stream << vs.copy_from(h_velocities.data());
    // Fill dxs with zeros
    std::vector<Vector3> zero_dx(xs.size(), Vector3::Zero());
    stream << dxs.copy_from(zero_dx.data());

    masses = device.create_buffer<Float>(h_masses.size());
    stream << masses.copy_from(h_masses.data());

    thicknesses = device.create_buffer<Float>(h_thicknesses.size());
    stream << thicknesses.copy_from(h_thicknesses.data());

    // 2) Elements
    codim_0ds = device.create_buffer<IndexT>(h_codim_0ds.size());
    stream << codim_0ds.copy_from(h_codim_0ds.data());

    codim_1ds = device.create_buffer<Vector2i>(h_codim_1ds.size());
    stream << codim_1ds.copy_from(h_codim_1ds.data());
    rest_lengths = device.create_buffer<Float>(codim_1ds.size());

    codim_2ds = device.create_buffer<Vector3i>(h_codim_2ds.size());
    stream << codim_2ds.copy_from(h_codim_2ds.data());
    rest_areas = device.create_buffer<Float>(codim_2ds.size());

    tets = device.create_buffer<Vector4i>(h_tets.size());
    stream << tets.copy_from(h_tets.data());
    rest_volumes = device.create_buffer<Float>(tets.size());

    // 3) Material Space Attribute
    // Rod
    Kernel1D rod_basis_kernel = [&](BufferView<Vector2i> codim_1ds, 
                                    BufferView<const Vector3> x_bars,
                                    BufferView<Float> rest_lengths) {
        set_block_size(256);
        Var i = dispatch_x();
        Var edge = codim_1ds.read(i);
        Var x0 = x_bars.read(edge[0]);
        Var x1 = x_bars.read(edge[1]);
        rest_lengths.write(i, length(x1 - x0));
    };

    auto rod_shader = device.compile(rod_basis_kernel);
    stream << rod_shader(codim_1ds.view(), x_bars.view(), rest_lengths.view())
                  .dispatch(codim_1ds.size());

    // Shell
    Kernel1D shell_basis_kernel = [&](BufferView<Vector3i> codim_2ds,
                                      BufferView<const Vector3> x_bars,
                                      BufferView<Float> rest_areas) {
        set_block_size(256);
        Var i = dispatch_x();
        Var tri = codim_2ds.read(i);
        Var x0 = x_bars.read(tri[0]);
        Var x1 = x_bars.read(tri[1]);
        Var x2 = x_bars.read(tri[2]);
        Var E01 = x1 - x0;
        Var E02 = x2 - x0;
        rest_areas.write(i, 0.5f * length(cross(E01, E02)));
    };

    auto shell_shader = device.compile(shell_basis_kernel);
    stream << shell_shader(codim_2ds.view(), x_bars.view(), rest_areas.view())
                  .dispatch(codim_2ds.size());

    // FEM3D Material Basis
    Dm3x3_invs = device.create_buffer<Matrix3x3>(tets.size());

    Kernel1D fem3d_material_basis_kernel = [&](BufferView<Vector4i> tets,
                                                BufferView<const Vector3> x_bars,
                                                BufferView<Matrix3x3> Dm3x3_inv,
                                                BufferView<Float> rest_volumes) {
        set_block_size(256);
        Var i = dispatch_x();
        Var tet = tets.read(i);
        Var x0 = x_bars.read(tet[0]);
        Var x1 = x_bars.read(tet[1]);
        Var x2 = x_bars.read(tet[2]);
        Var x3 = x_bars.read(tet[3]);

        // Compute Dm_inv using fem utils
        Dm3x3_inv.write(i, fem::Dm_inv(x0, x1, x2, x3));
        
        // Compute rest volume
        Var Ds = fem::Ds(x0, x1, x2, x3);
        Var V = determinant(Ds) / 6.0f;
        // Note: In a real implementation, we'd need proper assertion handling
        // For now, we just compute the volume
        rest_volumes.write(i, V);
    };

    auto fem3d_shader = device.compile(fem3d_material_basis_kernel);
    stream << fem3d_shader(tets.view(), x_bars.view(), Dm3x3_invs.view(), rest_volumes.view())
                  .dispatch(tets.size());
}
...
{
    stream << xs.copy_to(h_positions.data());
}
