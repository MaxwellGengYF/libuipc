...
luisa::compute::BufferView<Vector3> Codim1DConstitutionDiffParmReporter::DiffParmInfo::xs() const noexcept
{
    return m_impl->fem().xs.view();  // must return full buffer, because the indices index into the full buffer
}

luisa::compute::BufferView<Vector3> Codim1DConstitutionDiffParmReporter::DiffParmInfo::x_bars() const noexcept
{
    return m_impl->fem().x_bars.view();  // must return full buffer, because the indices index into the full buffer
}

luisa::compute::BufferView<Float> Codim1DConstitutionDiffParmReporter::DiffParmInfo::rest_lengths() const noexcept
{
    auto& info = constitution_info();
    return m_impl->fem().rest_lengths.view(info.primitive_offset, info.primitive_count);
}

luisa::compute::BufferView<Float> Codim1DConstitutionDiffParmReporter::DiffParmInfo::thicknesses() const noexcept
{
    return m_impl->fem().thicknesses.view();
}

luisa::compute::BufferView<IndexT> Codim1DConstitutionDiffParmReporter::DiffParmInfo::is_fixed() const noexcept
{
    return m_impl->fem().is_fixed.view();  // must return full buffer, because the indices index into the full buffer
}

luisa::compute::BufferView<Vector2i> Codim1DConstitutionDiffParmReporter::DiffParmInfo::indices() const noexcept
{
    auto& info = constitution_info();
...
luisa::compute::BufferView<Float> Codim1DConstitutionDiffParmReporter::DiffParmInfo::pGpP() const
{
