...
class MASPreconditionerEngine
{
  ...
  private:
    ...
    luisa::compute::Buffer<Int2>         level_sizes;
    luisa::compute::Buffer<int>          coarse_space_tables;
    luisa::compute::Buffer<int>          prefix_original;
    luisa::compute::Buffer<int>          prefix_sum_original;
    luisa::compute::Buffer<int>          going_next;
    luisa::compute::Buffer<int>          dense_level;
    luisa::compute::Buffer<LevelTable>   coarse_tables;
    luisa::compute::Buffer<unsigned int> fine_connect_masks;
    luisa::compute::Buffer<unsigned int> next_connect_masks;
    luisa::compute::Buffer<unsigned int> next_prefixes;
    luisa::compute::Buffer<unsigned int> next_prefix_sums;

    // ---- GPU buffers: neighbor graph ----
    int m_neighbor_list_size = 0;
    luisa::compute::Buffer<unsigned int> neighbor_lists;
    luisa::compute::Buffer<unsigned int> neighbor_starts;
    luisa::compute::Buffer<unsigned int> neighbor_nums;
    luisa::compute::Buffer<unsigned int> neighbor_lists_init;
    luisa::compute::Buffer<unsigned int> neighbor_nums_init;

    // ---- GPU buffers: partition mappings ----
    luisa::compute::Buffer<int> part_to_real;   // partition-ordered index -> real vertex index
    luisa::compute::Buffer<int> real_to_part;   // real vertex index -> partition-ordered index

    // ---- GPU buffers: cluster matrices ----
    luisa::compute::Buffer<ClusterMatrixSym>  cluster_hessians;   // assembled Hessian blocks (double)
    luisa::compute::Buffer<ClusterMatrixSymF> cluster_inverses;   // inverted preconditioner (float)

    // ---- GPU buffers: multi-level residual / solution ----
    luisa::compute::Buffer<Eigen::Vector3f> multi_level_R;
    luisa::compute::Buffer<float3>          multi_level_Z;
