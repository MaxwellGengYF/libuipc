...
luisa::compute::BufferView<Vector3> FiniteElementAnimator::BaseInfo::xs() const noexcept
{
    return m_impl->finite_element_method->xs();
}

luisa::compute::BufferView<Vector3> FiniteElementAnimator::BaseInfo::x_prevs() const noexcept
{
    return m_impl->finite_element_method->x_prevs();
}

luisa::compute::BufferView<Float> FiniteElementAnimator::BaseInfo::masses() const noexcept
{
    return m_impl->finite_element_method->masses();
}

luisa::compute::BufferView<IndexT> FiniteElementAnimator::BaseInfo::is_fixed() const noexcept
{
    return m_impl->finite_element_method->is_fixed();
}

luisa::compute::BufferView<Float> FiniteElementAnimator::ComputeEnergyInfo::energies() const noexcept
{
    auto [offset, count] = m_impl->constraint_energy_offsets_counts[m_index];
    return m_energies.subview(offset, count);
}

FEMLinearSubsystem::MutableDoubletVector3 FiniteElementAnimator::ComputeGradientHessianInfo::gradients() const noexcept
{
    auto [offset, count] = m_impl->constraint_gradient_offsets_counts[m_index];
    return m_gradients.subview(offset, count);
}

FEMLinearSubsystem::MutableTripletMatrix3x3 FiniteElementAnimator::ComputeGradientHessianInfo::hessians() const noexcept
{
